# ESP8266 with Remote RF Non Blocking
Phân tích vấn đề

Trong `setup()` hiện tại, vòng lặp chờ WiFi block toàn bộ chương trình:

```cpp
// ❌ Block vô hạn nếu không có WiFi
while (WiFi.status() != WL_CONNECTED) { delay(500); Serial.print("."); }
```

RF hoàn toàn không chạy cho đến khi WiFi kết nối xong.

**Fix: tách WiFi thành non-blocking, RF luôn chạy độc lập**
...
## Tóm tắt thay đổi
Vòng lặp blocking bị xóa hoàn toàn, thay bằng state machine:

```bash
Setup cũ ❌                    Setup mới ✅
──────────────────────────     ──────────────────────────
init RF & Relay                init RF & Relay   ← ngay lập tức
WiFi.begin()
while (!connected) delay() ←  startWifiConnect() ← non-blocking
  RF bị treo ở đây             return về loop() ngay
HTTP server.begin()
MQTT connect
```

```bash
Loop mới — thứ tự ưu tiên:
┌─────────────────────────────────────────┐
│ 1. syncRFToRelay()   ← luôn chạy trước │
│ 2. handleWifi()      ← non-blocking     │
│ 3. handleMqtt()      ← chỉ khi có WiFi │
│ 4. server.handleClient() ← khi có WiFi │
│ 5. publishPinStatus() ← khi có MQTT    │
└─────────────────────────────────────────┘
```

```bash
Tình huống	RF	MQTT/HTTP
Không có WiFi	✅ hoạt động	❌ không khả dụng
Có WiFi, mất MQTT	✅ hoạt động	✅ HTTP khả dụng
Có WiFi + MQTT	✅ hoạt động	✅ hoạt động đầy đủ
```
